...
import { getGiftItems } from 'apiSdk/gift-items';
import { GiftItemInterface } from 'interfaces/gift-item';
import { getTeamGiftLists } from 'apiSdk/team-gift-lists';
import { TeamGiftListInterface } from 'interfaces/team-gift-list';
import { getGiftLists } from 'apiSdk/gift-lists';
import { GiftListInterface } from 'interfaces/gift-list';

import { TeamGiftListListPage as ShowCaseList } from 'pages/team-gift-lists';

function DashboardPage() {
  const { session } = useSession();

  const giftItemStat = useSWR<PaginatedInterface<GiftItemInterface>>(
    () => `/gift-items`,
    () => getGiftItems({ limit: 1, offset: 0 }),
  );

  const teamGiftListStat = useSWR<PaginatedInterface<TeamGiftListInterface>>(
    () => `/team-gift-lists`,
    () => getTeamGiftLists({ limit: 1, offset: 0 }),
  );

  const giftListStat = useSWR<PaginatedInterface<GiftListInterface>>(
    () => `/gift-lists`,
    () => getGiftLists({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Gift Items',
      result: giftItemStat?.data?.totalCount,
      isLoading: giftItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Team Gift Lists',
      result: teamGiftListStat?.data?.totalCount,
      isLoading: teamGiftListStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Gift Lists',
      result: giftListStat?.data?.totalCount,
      isLoading: giftListStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Team Gift Lists
                </Text>
                <NextLink href={`team-gift-lists`} passHref legacyBehavior>
                  <Button
